...
export default function InventoryModule() {
  ...
  const filterOptions = useMemo(() => {
    const baseOptions = getProductFilterOptions(products);
  ...
  const handleConfirmDelete = async (product) => {
    try {
      ...
      setToast({ type: "success", message: "Producto eliminado del inventario." });
    } catch (deleteError) {
      ...
      setToast({
        ...
        message:
          result.created || result.updated
            ? `Importacion completada. Creados: ${result.created}, actualizados: ${result.updated}, errores: ${result.errors}.`
            : "La plantilla no contiene filas validas para importar todavia.",
      });
    } catch (importError) {
      setToast({
        type: "error",
  ...
  return (
    <RoleLayout
      requiredRole="DUENO"
      title="Inventario"
      subtitle="Opera el catalogo comercial desde un solo lugar: productos, stock, precios, categorias e importaciones."
    >
      <section className="space-y-6">
        <CommercialHero
          title="Centro operativo de inventario"
          description="Aqui se concentra el catalogo comercial con altas, ediciones, activacion, stock, filtros, importacion y exportacion para la operacion diaria."
          actions={
            <>
        ...
        <div className="grid gap-4 xl:grid-cols-[1.5fr,1fr]">
          <div className="rounded-3xl border border-[#E5E7EB] bg-[#FCFCFD] p-4 text-sm text-[#4B5563]">
            <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <p>
                Mostrando <span className="font-semibold text-[#1F2933]">{sortedProducts.length}</span> registros
                filtrados de un total de <span className="font-semibold text-[#1F2933]">{products.length}</span>.
              </p>
              <p>
                Pagina <span className="font-semibold text-[#1F2933]">{page}</span> de{" "}
          ...
          <div className="rounded-3xl border border-[#D7E7FF] bg-[#F6FAFF] p-4 text-sm text-[#4B5563]">
            <p className="font-semibold text-[#1F2933]">Variables clave del inventario</p>
            <div className="mt-2 space-y-1">
              <p>Stock, precio y estado ya estan integrados en el flujo principal.</p>
              <p>
                Categorias activas disponibles para clasificacion:
                <span className="font-semibold text-[#1F2933]"> {categories.length}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <CommercialPanel
          title="Centro de importacion y exportacion"
          subtitle="La plantilla oficial conserva una estructura unica para altas y actualizaciones masivas."
          actions={
            <>
        ...
        >
          <div className="px-6 py-5 text-sm text-[#4B5563]">
            <p>Encabezado oficial de la plantilla:</p>
            <span className="mt-3 block rounded-2xl bg-[#F6FAFF] px-3 py-3 font-mono text-xs text-[#1D6FD1]">
              {officialHeaders}
            </span>
          </div>
        </CommercialPanel>
        ...
        {lastImportSummary ? (
          <CommercialPanel title="Ultima importacion" subtitle="Resumen del archivo procesado y observaciones mas importantes.">
        ...
        {error ? (
          <section className="rounded-3xl border border-[#FECACA] bg-[#FFF1F2] p-6 text-[#BE123C] shadow-[0_18px_40px_rgba(190,24,93,0.08)]">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold">No se pudo cargar el modulo de inventario</p>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2 text-sm text-[#6B7280]">
            <Boxes size={16} />
            <span>Inventario listo para seguir creciendo con movimientos, proveedores y trazabilidad.</span>
          </div>
      ...
      <ProductFormModal
        key={`${formMode}-${editingProduct?.id || "new"}-${isFormOpen ? "open" : "closed"}`}
        open={isFormOpen}
        mode={formMode}
